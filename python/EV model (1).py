...
def get_car_from_index(index):
	...
	res.append(str(df[df.Index == index]['Release'].values[0]) + ' ' + df[df.Index == index]['Brand'].values[0] + ' ' + df[df.Index == index]['Model'].values[0]+ ' ' + '$' + str(df[df.Index == index]['Price'].values[0]) + ' ' + str(df[df.Index == index]['Range'].values[0]) + 'km)
	return res
...
car_index = get_index_from_id(car_user_likes)
...
i=0
for element in sorted_similar_cars[1:]:
		print(get_car_from_index(element[0]))
		i=i+1
		if i>2:
			break